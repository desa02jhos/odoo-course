...
class ControlThreshold(models.Model):
    ...
    country_origin_id = fields.Many2one('res.country', 'País Origen', required=True, track_visibility='onchange')
    partner_origin_id = fields.Many2one('res.partner','Corresponsal Origen',require=True,
        domain=['&',('is_company','=',True),('category_id.keycode','=','correspondent')], track_visibility='onchange')
    country_destination_id = fields.Many2one('res.country',string=u'País Destino',required=True, track_visibility='onchange')
    partner_destination_id = fields.Many2one('res.partner',string=u'Corresponsal Destino',required=True, track_visibility='onchange')
    s_threshold_type = fields.Selection([('obl','Obligatorio'),('fac','Facultativo')],'Tipo de Umbral',required=True, track_visibility='onchange')
    ...
    s_control_type = fields.Selection([('U','Operación única'),('M','Operación múltiple o acumulada')],
    ...
    s_time_frame_type = fields.Selection(
        [
            ('cro', 'Cronológico'),
    ...
    b_authorization = fields.Boolean('Requiere Autorización',default=False)
...
# Documentos necesarios en el PLAFT
...
class CommercialThreshold(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', 'Socio de Negocio Nacional',
        domain=['&',('category_id','=',1), ('country_id.code','=','PE')])
    currency_id = fields.Many2one('res.currency', 'Moneda',
        domain=[('name','in',['PEN','USD','EUR'])])
